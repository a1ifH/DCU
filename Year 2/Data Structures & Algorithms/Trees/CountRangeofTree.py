...
class BST:
    ...
    def recursive(self, ptr, lo, hi):
        if ptr == None:
            return 0
        if ptr.item >= lo and ptr.item <= hi:
            return self.recursive(ptr.left, lo, hi) + self.recursive(ptr.right, lo, hi) + 1
        else:
            return self.recursive(ptr.left, lo, hi) + self.recursive(ptr.right, lo, hi)
                
    def count(self, hi, lo):
        return self.recursive(self.root, hi, lo)    