...
class BST:
    ...
    def is_present(self, other):
        return self.recursive(self.root, other)

    def recursive(self, ptr, other):
        if ptr == None:
            return False
        if ptr.item == other:
            return True
        else:
            return self.recursive(ptr.left, other) or self.recursive(ptr.right, other)